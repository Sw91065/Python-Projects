...
/* Solution */
select branch,sum(revenue-cost) as rp from bank_branch_pl where cost< revenue group by branch;
# Question 2:
# 2)  Find the average Estimated_ sale_price for each product  
/* Solution */
select avg(estimated_sale_price),product from bank_inventory_pricing group by product;
# Question 3:
# 3)  Find the  month wise total quantity of each product
/* Solution */
select month,product,sum(quantity) from bank_inventory_pricing group by Product;
# Question 4:
# 4)  Find the maximum profit(Estimated_sale_price-Purchase_cost) for each product (Hint: Do not display loss).
/* Solution */
select product,max(Estimated_sale_price-Purchase_cost) as profit from bank_inventory_pricing group by product having profit>0;
# Question 5:
# 5)  Find the minimum revenue for each banker if estimated_profit is greater than 20000000.
/* Solution */
select banker ,min(revenue) as rev from bank_branch_pl where estimated_profit > 20000000 group by banker;
/* Pre-requisite */
...
select branch, sum(revenue-cost) as real_profit , estimated_profit from bank_branch_pl group by branch having real_profit>estimated_profit;
# Question 8:
...
select branch, sum(revenue-cost) as real_profit , estimated_profit from bank_branch_pl group by branch having real_profit>0 and  estimated_profit>0;
# Question 9:
...
select branch, sum(revenue-cost) as real_profit , estimated_profit from bank_branch_pl group by branch having real_profit<0;
# Question 10:
...
select branch, min(revenue-cost),product from bank_branch_pl group by branch,product having min(revenue-cost)>0;
# Question 11:
...
select branch,product,revenue-cost as real1,estimated_profit  from bank_branch_pl group by product having real1>estimated_profit;
# Question 12:
...
select product,revenue-cost as real1,estimated_profit from bank_branch_pl group by product having real1 between estimated_profit*0.9 and estimated_profit*1.1;
# Question 13:
...
select branch,product,sum(revenue-cost) as real1,sum(estimated_profit) as ep,sum(cost) as ct  from bank_branch_pl group by product having real1>ep and real1>ct*0.3;